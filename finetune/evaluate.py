...
def layout_from_source_ids(source_ids: list) -> tuple[list[dict], list[str]]:
    """모델이 낸 source_id 목록을 레이아웃 항목으로 바꾼다(--layout-target 학습용).

    격자 좌표는 모델이 아니라 playbook.tiling_for가 계산한다 — 패널 개수와 순번만으로
    정해지는 기하학이라 모델이 낼 이유가 없고, 좌표를 모델이 내면 겹침·빈칸을 구조적으로
    막을 수 없기 때문이다.

    반환값은 (레이아웃, 버려진 id 목록). 카탈로그에 없는 id는 실제 화면이 아니므로
    버리고 따로 세어 둔다 — 모델이 화면 이름을 지어내는 빈도가 이 방식을 실제로 쓸 수
    있는지 판단하는 핵심 수치다.
    """
    ...
    for sid in source_ids:
        ...
        if entry and sid not in [v["source_id"] for v in valid]:
            valid.append({"source_id": sid, "name": entry["name"]})
        else:
            invalid.append(sid)

    slots = pb.tiling_for(len(valid))
    layout = [
        {**item, "priority": i + 1, "grid": list(slots[i])}
        for i, item in enumerate(valid[: len(slots)])
    ]
...
def evaluate(turns: list[dict], backend: object, limit: int | None = None) -> dict:
    ...
    for turn in turns:
        ...
        # 지표② — 화면 배치를 전문가 정답 레이아웃과 비교한다. 모델이 배치를 직접
        # 냈으면 그걸 그대로 채점하고(원안 구조), 안 냈으면 예측한 상황 유형으로
        # 플레이북 레이아웃을 만들어 채점한다(현행 구조).
        if predicted_layout is not None:
            layout, invalid_ids = layout_from_source_ids(predicted_layout)
            layout_turns += 1
