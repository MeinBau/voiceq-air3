...
def build() -> Deck:
    ...
    d.content(
        ...
        "AI가 지휘소에서 틀렸을 때",
        [("거짓 정보 생성 (환각)",
          "화면 이름과 좌표를 모델이 지어낼 수 없게 폐쇄 카탈로그를 거치도록 강제했습니다. "
          "270건의 실재하는 화면 소스 중에서만 선택됩니다."),
         ("형식 이탈 · 성능 저하",
          "JSON 강제 + 코드펜스 제거 + 파싱 실패 시 재시도. 사태 ID 표기가 흔들려도 정규화해 "
          "매칭합니다(실측 JSON 유효율 100%). 모델 Drift는 월간 모니터링과 분기별 재학습으로 대응하며, "
          "평가 하네스가 이미 자동화되어 있습니다."),
         ("그리고 — 결심은 사람이 합니다",
          "본 체계는 표출·기록 보조에 한정됩니다. 무기체계 통제·사격 지시 등 결심·타격 기능은 "
          "개발 범위에서 명시적으로 제외했습니다.")],
    )
