...
| 기획서 항목 | 요구 | 구현 | 상태 |
|---|---|---|---|
| 3-나 1단계 | 학습 데이터 500건 이상 | `gen_dataset.py` | ✅ **1,546건** 생성 |
...
| 4-다② 모델 학습 | Qwen2.5 / Llama-3, LoRA | `train_lora.py` (QLoRA) | ✅ 스크립트 완성, 학습은 GPU 환경 필요 |
...
|---|---|---|---|---|
| train | 176 | 1,265 | 2,530 | 11 / 11 |
| valid | 22 | 125 | 250 | 11 / 11 |
| test | 22 | 156 | 312 | 11 / 11 |
| **합계** | **220** | **1,546** | **3,092** | |

일지 라벨 분포(train): 상황 252 · 조치 872 · 무시 141. "발언 하나 = 사태 하나가 아니다"를
...
| 경로 | median | p95 | max |
|---|---|---|---|
| FAST | 1,188 | 1,226 | 1,272 |
| FULL | 1,549 | 1,669 | 1,764 |

train+valid p99 = 1,689 → `--max-seq-len 2048`로 잘림 없음.
...
| 백엔드 | 상황유형 정확도 | COP 완전일치 | 일지 kind | 키워드 정확도 | 누락률 |
|---|---|---|---|---|---|
| `gold` (정답 그대로) | 100% | 100% | 100% | 100% | 0% |
| `perturb --noise 0.3` | 69.9% | 78.2% | 73.1% | 91.7% | 9.3% |

`gold`에서 전 지표 100%가 나오므로 지표 계산식 자체는 정상이고, `perturb`에서 유의미하게
하락하므로 오류를 실제로 잡아낸다. `perturb`에서 COP 완전일치(78.2%)가 상황유형
정확도(69.9%)보다 높은 것은 정상이다 — 여러 상황 유형이 "해당 지역 cctv" 슬롯만 갖고 있어
유형을 틀려도 같은 레이아웃이 나오는 경우가 있기 때문이다.

## 5. 학습 환경 — 로컬에서는 돌릴 수 없다
...
|---|---|---|
| Colab T4 (무료) | 16GB | 가능, 대략 1.5~2.5시간 |
| Colab L4 / A100 | 24~40GB | 30~60분 |
