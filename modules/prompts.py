...
- 화면을 직접 고르지 마십시오. 어떤 화면을 어느 순서로 띄울지는 운용자가 만든
  ...
  · focus_cell : 이 상황이 벌어지고 있는 격자 좌표 하나(A1~J7).
             이 값으로 "해당 지역 CCTV"가 자동 선택되므로 정확해야 합니다.
             위치를 알 수 없으면 기지 중앙 "E4"로 두십시오.
...
- tracks: 현재 식별된 항적/접촉의 전체 목록입니다. 매번 "현재 존재하는 것 전부"를 출력하세요.
